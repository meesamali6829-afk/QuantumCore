...
# 1. DATABASE CONFIG (RENDER COMPATIBILITY FIX)
# Ye line check karegi ke agar Render ka Postgres available hai toh wo use kare, warna SQLite
uri = os.getenv("DATABASE_URL")
if uri and uri.startswith("postgres://"):
    uri = uri.replace("postgres://", "postgresql://", 1)

app.config['SQLALCHEMY_DATABASE_URI'] = uri or 'sqlite:///quantumcore.db'
...
def send_quantum_alert(receiver_email, p_name, data_payload, is_owner_copy=False):
    ...
    # Tip: Render settings mein EMAIL_PASS key bana kar password wahan rakhna zyada safe hai
...
# --- ADMIN VIEW ROUTE (DB ka data dekhne ke liye) ---
@app.route('/quantum/admin/debug')
def admin_debug():
    users = User.query.all()
    vaults = DataVault.query.all()
    res = "<h1>QuantumCore Admin Panel</h1>"
    res += f"<h3>Total Users: {len(users)}</h3><ul>"
    for u in users: res += f"<li>{u.email} (Plan: {u.plan})</li>"
    res += "</ul><h3>Recent Vault Logs:</h3><ul>"
    for v in vaults: res += f"<li>Project ID {v.project_id}: {v.payload} (IP: {v.ip_address})</li>"
    res += "</ul>"
    return res
