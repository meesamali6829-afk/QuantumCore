...
# 1. DATABASE CONFIG (NEW RENDER POSTGRES CONNECTED)
# Aapka diya gaya updated connection string yahan hai:
uri = os.getenv("DATABASE_URL")
EXTERNAL_DB_URL = "postgresql://quantum_vault_db_user:UntsYrD6bN9bhxFfilR0cW8tYS8T4GkZ@dpg-d6k3bqi4d50c73d8mpn0-a.oregon-postgres.render.com/quantum_vault_db"
...
try:
    ...
    # Agar mobile par drivers nahi hain toh local backup use hoga
...
class Project(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    project_name = db.Column(db.String(100)) # Individual Project Identification [cite: 2026-02-20]
...
class DataVault(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    project_id = db.Column(db.Integer, db.ForeignKey('project.id')) # Data Segregation [cite: 2026-02-20]
...
def send_quantum_alert(receiver_email, p_name, data_payload):
    ...
    body = f"<h2>Project: {p_name}</h2><p>Data successfully captured and stored.</p><pre>{data_payload}</pre>"
...
# 4. ROUTES (Worldwide Access Enabled [cite: 2026-02-20])
@app.route('/')
def index():
...
def receive_data(api_key):
    ...
    # Master and User Copy Alerts [cite: 2026-02-20]
